...
const components: string[][] = require(path.join(
  process.cwd(),
  "components.json"
));
const resolved_graph: Graph = require(path.join(
  process.cwd(),
  "resolved_graph.json"
));
...
const indexedComponent: IndexComponents = new Map(
  components
    .map((o) => o.map<[string, string[]]>((oo) => [oo, o]))
    .reduce((p, n) => [...p, ...n], [])
);

function getLinksFrom(resolved_graph: Graph, name: string): string[] {
  return resolved_graph.links
    .filter((l) => l.source === name)
    .map((l) => l.target);
}

function getMapEntryFor(
...
const targets: IndexedTargets = new Map(
  resolved_graph.nodes.map((n) => getMapEntryFor(resolved_graph, n))
);
